...
class _WishlistApprovalContent extends StatelessWidget {
  ...
  Future<void> _refresh(BuildContext context) async {
    ...
    _showMessage(context, context.l10n.failedToRefreshWishes);
  }
  ...
  Future<void> _approveWish(
    ...
    final result = await context.read<WishlistApprovalController>().approveWish(
      wishId: wishId,
  ...
  Future<void> _rejectWish(
    ...
    final result = await context.read<WishlistApprovalController>().rejectWish(
      wishId: wishId,
    );
  ...
  void _showActionError(BuildContext context, WishlistActionResult result) {
    final message = switch (result.errorCode) {
      WishlistActionErrorCode.approveFailed => context.l10n.failedToApproveWish,
      WishlistActionErrorCode.rejectFailed => context.l10n.failedToRejectWish,
      null => null,
    };
  ...
  Widget build(BuildContext context) {
    return WishlistApprovalView(
      ...
      onApprove: ({required wishId, required targetPoints}) {
        return _approveWish(
          context,
          wishId: wishId,
          targetPoints: targetPoints,
      ...
      onReject: ({required wishId}) {
        return _rejectWish(context, wishId: wishId);
      },
    );
  }
}
