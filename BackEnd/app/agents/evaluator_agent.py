...
class EvaluatorAgent:
    ...
    def evaluate(
        ...
        for attempt in range(max_attempts):
            ...
            if result is None:
                validation_errors = [
                    "No valid structured evaluation was returned."
                ]
                continue
        ...
        raise ValueError(
            "EvaluatorAgent failed to produce a valid "
            "evaluation after 3 attempts."
        )
...
PREVIOUS EVALUATION ATTEMPT WAS REJECTED
BY THE EVALUATION VALIDATOR.

VALIDATION ERRORS:
{errors_text}

Correct every validation error in the new evaluation.
"""
...
- Create a separate issue for each meaningful problem.

- Assign each issue to the agent that is actually
  responsible for fixing that root problem.

- Do not assign every issue to STRATEGY by default.

- Use HIGH severity for safety problems, serious
  age-inappropriateness, or major unsupported assumptions.

- Use MEDIUM for meaningful quality problems that should
  be revised before showing the plan.

- Use LOW only for minor concerns.
    ...
    def _validate_evaluation(
        ...
        for issue in evaluation.issues:
            if issue.issue_type not in valid_issue_types:
                raise ValueError(
                    "EvaluatorAgent returned an invalid "
        ...
        if evaluation.approved != expected_approved:
            raise ValueError(
                "EvaluatorAgent approval state is inconsistent "
                "with the score and issue severity."
            )