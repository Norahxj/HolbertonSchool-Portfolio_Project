...
class WishlistService:
    ...
    def create_wish(self, child_id, wish_data):
        try:
            ...
            pending_count = (
                self.wishlist_repository.get_pending_count_by_child_id(child_id)
            )
            if pending_count >= 5:
                db.session.rollback()
    ...
    def approve_wish(self, wish_id, parent_id, target_points):
        ...
        try:
            ...
            child = self.child_repository.get_child_for_guardian(wish.child_id, parent_id)
            if not child:
                db.session.rollback()
                return None, "child_not_found"
            locked_child = (
                self.child_repository.get_child_by_id_for_update(wish.child_id)
            )
            if not locked_child:
                db.session.rollback()
                return None, "child_not_found"
            wish = (
                self.wishlist_repository.get_wish_by_id_for_update(wish_id)
            )
            if not wish:
                db.session.rollback()
                return None, "wish_not_found"
            ...
            approved_count = (
                self.wishlist_repository.get_approved_count_by_child_id(wish.child_id)
            )
            if approved_count >= 3:
                db.session.rollback()
    ...
    def reject_wish(self, wish_id, parent_id):
        try:
            ...
            if not child:
                return None, "child_not_found"
            wish = self.wishlist_repository.get_wish_by_id_for_update(wish_id)
            if not wish:
                db.session.rollback()
                return None, "wish_not_found"
            if wish.status != "PENDING":
    ...
    def achieve_wish(self, wish_id, child_id):
        try:
            wish = (
                self.wishlist_repository.get_wish_for_child_for_update(wish_id, child_id)
            )
            if not wish:
                db.session.rollback()
                return None, "wish_not_found"
            ...
            if (
                wish.target_points is None or wish.target_points <= 0
            ):
                db.session.rollback()
                return None, "invalid_target_points"
            points_record = (
                self.points_repository.get_points_by_child_id_for_update(child_id)
            )
